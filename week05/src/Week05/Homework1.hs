...
mkPolicy pkh deadline ctx = traceIfFalse "signature missing"   (txSignedBy info pkh)     &&
                            traceIfFalse "deadline has passed" (to deadline `contains` txInfoValidRange info)
  where
    info :: TxInfo
